...
<head>
    ...
    <script>
        ...
        const SUPABASE_URL = 'https://lqmvsxphvwnnadnhipmx.supabase.co'
        const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxxbXZzeHBodndubmFkbmhpcG14Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTQyNDU1MTQsImV4cCI6MjA2OTgyMTUxNH0.otpNDL2gVlnDEuB_4y8dhxNM5ymrazArCJzoz-fcDIs'
        const SUPABASE_SERVICE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxxbXZzeHBodndubmFkbmhpcG14Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1NDI0NTUxNCwiZXhwIjoyMDY5ODIxNTE0fQ.u1VXjZTzQ6GFDAakwPhrem1N75a14LLa39bi2g7BIHs'
...
<body class="bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white transition-all duration-300">
    ...
    <script>
        // 🚀 Application state avec Supabase
        ...
        // 🔥 FONCTIONS SUPABASE - Gestion des utilisateurs
        ...
        async function loadUserSession(user) {
            ...
            document.getElementById('currentUserDisplay').textContent = `Connecté: ${user.username}`;
            ...
            await renderTradesGroupedByDate();
        }
            ...
            try {
                ...
                if (error) {
                    if (error.code === '23505') { // Unique constraint violation
                        ...
                        showCustomAlert('Erreur lors de la création: ' + error.message, 'error');
                    }
                    return;
                }
        ...
        async function deleteUser(userId) {
            ...
            modal.innerHTML = `
                <div class="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg shadow-lg max-w-sm w-full mx-4">
                    <p class="text-gray-700 dark:text-gray-300 mb-4 text-sm md:text-base">Supprimer cet utilisateur ?</p>
                    <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3">
                        ...
                        <button onclick="confirmDeleteUser('${userId}'); this.closest('.fixed').remove();" class="px-4 py-2 bg-red-500 text-white hover:bg-red-600 rounded transition-colors text-sm">Supprimer</button>
                    </div>
                </div>
            `;
        ...
        // 🔥 FONCTIONS SUPABASE - Gestion des trades
                    ...
                    .insert([
                        {
                            ...
                            pnl: tradeData.pnl ? parseFloat(tradeData.pnl) : 0,
                            notes: tradeData.tradeNotes || null,
                            screenshots: tradeData.screenshots || null
                        }
                    ]);
                
                if (error) {
                    console.error('Erreur ajout trade:', error);
                    showCustomAlert('Erreur lors de l\'ajout du trade', 'error');
                    return;
                }
        ...
        async function deleteTrade(tradeId) {
            ...
            modal.innerHTML = `
                <div class="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg shadow-lg max-w-sm w-full mx-4">
                    ...
                    <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3">
                        <button onclick="this.closest('.fixed').remove()" class="px-4 py-2 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded transition-colors text-sm">Annuler</button>
                        <button onclick="confirmDeleteTrade('${tradeId}'); this.closest('.fixed').remove();" class="px-4 py-2 bg-red-500 text-white hover:bg-red-600 rounded transition-colors text-sm">Supprimer</button>
                    </div>
                </div>
            `;
            document.body.appendChild(modal);
        ...
        // Statistics and chart management
        async function updateStatistics() {
            if (!currentUserData) return;
            
            const trades = await getCurrentTrades();
            const initialBalance = currentUserData.initial_balance || 10000;
        ...
        async function updateChart() {
            if (!chart || !currentUserData) return;
        ...
        // Render trades table
        async function renderTradesGroupedByDate() {
                    ...
                    html += `
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                            ...
                            <td class="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap text-xs md:text-sm">
                                <span class="px-1.5 md:px-2 py-0.5 md:py-1 text-xs font-medium rounded-full ${trade.trade_type === 'Long' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200'}">
                                    ${trade.trade_type}
                                </span>
                            </td>
                            <td class="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap text-xs md:text-sm text-gray-900 dark:text-white">
                ...
                <div class="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-xl shadow-lg max-w-2xl w-full mx-2 md:mx-4 max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 mb-4 md:mb-6">
                        ...
                        <div>
                            ...
                            <p class="text-gray-900 dark:text-white font-medium text-sm md:text-base">${trade.currency_pair}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Type</label>
                            <span class="px-2 py-1 text-xs font-medium rounded-full ${trade.trade_type === 'Long' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200'}">
                                ${trade.trade_type}
                            </span>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Taille Position</label>
                            <p class="text-gray-900 dark:text-white text-sm md:text-base">${trade.position_size}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Prix d'entrée</label>
                            <p class="text-gray-900 dark:text-white text-sm md:text-base">${trade.entry_price}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Stop Loss</label>
                            <p class="text-gray-900 dark:text-white text-sm md:text-base">${trade.stop_loss || 'N/A'}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Take Profit</label>
                            <p class="text-gray-900 dark:text-white text-sm md:text-base">${trade.take_profit || 'N/A'}</p>
                        </div>
                        <div>
                    ...
                    ${trade.notes ? `
                        <div class="mb-4 md:mb-6">
                            ...
                            <div class="bg-gray-50 dark:bg-gray-700 p-3 md:p-4 rounded-lg">
                                <p class="text-gray-900 dark:text-white whitespace-pre-wrap text-sm md:text-base">${trade.notes}</p>
                            </div>
                        </div>
                    ` : ''}
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('settingsBtn').addEventListener('click', function() {
                if (currentUserData) {
                    document.getElementById('initialBalanceInput').value = currentUserData.initial_balance || 10000;
                }
            ...
            document.getElementById('saveSettings').addEventListener('click', async function() {
                ...
                if (newBalance && newBalance > 0 && currentUserData) {
                    try {
                        const { error } = await supabase
                            .from('users')
                        ...
                        if (error) {
                            showCustomAlert('Erreur lors de la sauvegarde', 'error');
                            return;
                        }
                        
                        currentUserData.initial_balance = newBalance;
                        await updateStatistics();
                        updateChart();
                        document.getElementById('settingsModal').classList.add('hidden');
            ...
            document.getElementById('changePasswordForm').addEventListener('submit', async function(e) {
                ...
                if (!currentUserData || oldPassword !== currentUserData.password) {
                ...
                try {
                    ...
                    if (error) {
                        showCustomAlert('Erreur lors du changement', 'error');
                        return;
                    }
